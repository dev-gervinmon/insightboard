...
export const get_sales_by_region = async (start_date, end_date) => {
  const response = await fetch(
    `http://127.0.0.1:8000/metrics/sales-by-region?start_date=${start_date}&end_date=${end_date}`
  );
...
export const get_sales_over_time_daily = async (start_date, end_date) => {
  const response = await fetch(
    `http://127.0.0.1:8000/metrics/sales-over-time/daily?start_date=${start_date}&end_date=${end_date}`
  );
  const data = await response.json();
  return data;
};
